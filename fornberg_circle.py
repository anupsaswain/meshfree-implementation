...
from bayona_circle import circle_boundary, curve_by_theta1, curve_equation_circle1
from bayona_method import separate_frame_nodes, clear_boundary, plot_scatter
...
def adjust_fornberg(interior_nodes, fixed_nodes, n_iterations, n_proximity, h, tolerance):
...
if __name__ == "__main__":
    box1 = Rectangle(2, 2, 0, 0)
    domain_nodes = fornberg_distribution(uniform, box1, 100)
    ...
    boundary_nodes = circle_boundary(curve_by_theta1, r, h)
    frame_nodes, interior_nodes = separate_frame_nodes(curve_equation_circle1, domain_nodes)
    fixed_points = np.append(frame_nodes, boundary_nodes, axis=0)
    interior_nodes = clear_boundary(interior_nodes, boundary_nodes, h)

    interior_nodes_adjusted = adjust_fornberg(interior_nodes, boundary_nodes, n_iterations, n_proximity, h, tolerance)

    frame_nodes2, interior_nodes_adjusted = separate_frame_nodes(curve_equation_circle1, interior_nodes_adjusted)

    temp_points = np.append(boundary_nodes, interior_nodes_adjusted, axis=0)
